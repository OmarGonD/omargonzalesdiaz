...
Como ya explicamos en un post anterior, una API proporciona diferentes URLs (el término correcto es `endpoints`) para hacer consultas a la Base de Datos de una aplicación e incluso transformar la data antes de devolverla a la aplicación que está solicitándo esta información. E incluso accediendo a un `endpoint` puedes eliminar la registros o completamente la base de datos asociada a un API.
